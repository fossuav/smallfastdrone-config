...
// End-to-end coverage of the catalogue + the frame-select wizard:
...
test('The catalogue lists every recipe; frame-select writes FRAME_CLASS+TYPE to SITL', async ({ page }) => {
  ...
  // One catalogue, reached directly from the nav (decision 43) — no
  // library page and no hop through the ribbon to find it.
  await page.getByRole('link', { name: 'Recipes', exact: true }).click()
  await expect(page.getByRole('heading', { name: 'Recipes', exact: true })).toBeVisible()

  // The guided walk keeps its own way in, above the grid.
  await expect(page.getByRole('link', { name: 'Open the guided bringup' })).toBeVisible()

  // Bringup steps are cards in the same catalogue, and each says which
  // step of the guided walk it is.
  const frameCardLink = page.getByRole('link', { name: /Open the Pick your frame recipe/ })
  await expect(frameCardLink).toBeVisible()
  await expect(frameCardLink.getByText('Bringup step 2')).toBeVisible()

  // Paid entries sit in the same catalogue, under one gating seam.
  await expect(page.getByRole('heading', { name: 'Pro recipes' })).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Pro PID tune' })).toBeVisible()

  // Open frame-select.
  await frameCardLink.click()
  ...
  // Returning to the catalogue shows the completion badge + the wizard's
  // dynamic outcome instead of the manifest's prospective outcome.
  await page.getByRole('button', { name: 'Done' }).click()
  await expect(page.getByRole('heading', { name: 'Recipes', exact: true })).toBeVisible()
  const frameCard = page.getByRole('link', { name: /Open the Pick your frame recipe/ })
...
test('The catalogue carries the locked-gating seam', async ({ page }) => {
  // No connection needed — the catalogue renders from the static wizard
  // registry and the locked card renders regardless of capability.
...
test('The retired library and field-tools URLs land where they meant', async ({ page }) => {
  // Decision 43 collapsed three catalogues into one. A bookmark from
  // before it should still arrive somewhere sensible rather than blank.
  await page.goto(`/wizard${SITL_QUERY}`)
  await expect(page).toHaveURL(/\/wizard\/bringup/)

  await page.goto(`/field${SITL_QUERY}`)
  await expect(page).toHaveURL(/\/recipes\?.*view=radio/)
  await expect(page.getByRole('heading', { name: 'Recipes', exact: true })).toBeVisible()
})
...
test('Bringup ribbon walks preflight + frame-select + connections + motor-check and marks itself complete', async ({ page }) => {
  ...
  // Pick Hex Plus, Apply, then "Done" returns to the ribbon (via
  // returnTo), which auto-advances to Motors.
  ...
  // All three sub-wizards complete → bringup auto-marks itself. It is the
  // catalogue's banner rather than a card (decision 43: a grid cannot
  // express an order, so the guided walk keeps its own way in), and the
  // banner carries the same Done badge and the watcher's outcome.
  await page.getByRole('link', { name: 'Recipes', exact: true }).click()
  const guided = page.getByRole('link', { name: 'Open the guided bringup' })
  // exact, because getByText is case-insensitive by default and the
  // banner's step counter reads "steps done".
  await expect(guided.getByText('Done', { exact: true })).toBeVisible({ timeout: 15_000 })
  await expect(guided.getByText(/All 4 bringup steps complete/)).toBeVisible()
})
