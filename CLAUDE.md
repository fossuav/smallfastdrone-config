...
- **Helper scripts encode current conventions — keep them current, and offer to.** `scripts/new-wizard.sh` (the `bun run new:wizard` scaffold) and `scripts/lua-check.sh` (the `bun run lua:check` applet-on-SITL check) bake in what we've learned about wizard structure and the dev workflow — the manifest shape, the DesktopView patterns (returnTo, markComplete, the mandatory visual), the Lua applet skeleton, the SITL verification dance. When a slice changes a convention one of them encodes, **proactively offer to fold the change back into the script in the same change**, so the next wizard starts from current truth, not a stale template. Same principle as `dev-setup.sh` and `README.md`: a learning that lives only in the code you just wrote evaporates. A repeated multi-step workflow that generates permission-prompt churn is a candidate for a new pre-authorised helper — propose one rather than re-running the dance by hand.
- **`README.md` reflects current reality, not aspirations.** When a slice adds a new command, capability, project-layout entry, or changes how something is run, update README.md in the same commit. The "What's here right now" section describes what actually works today; the rest of the doc explains how to use it. A new contributor reading just the README should be able to clone, set up, run, and test whatever slices have landed.
