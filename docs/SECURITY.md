...
| F11 | Owner key region in `.apsec_data` | `AP_CheckFirmware.h` — `struct ap_owner_data`; accessors alongside F1's | ⏳ **Not started.** Mirrors F1 exactly: own region, own 8-byte region signature, `find_owner_key()` / `owner_key_is_set()` / write-once `set_owner_key()` that refuses a zero key and refuses while armed. Placed after `ap_identity_data` for the same reason F1 went last — everything upstream is bounded by `AP_PUBLIC_KEY_MAX_KEYS` and cannot reach it. |
