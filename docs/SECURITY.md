...
| T7 | Views | SFD-enable and recovery wizards, both bringup-ribbon mountable. Enable is unblocked for its identity half (T1, T2, T6 in): drive `useSfdEnable()`, download `outcome.text` as `outcome.filename`, show the lock as "coming with a firmware update" until F6. Recovery waits on T3. |
