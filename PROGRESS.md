...
| Phase 0 — Scaffolding | 🔧 In progress | Slices done: 1 (scaffold), 3 (lint), 2 (UI lib), 4 (router+views), 5 (Pinia+expert mode), 7 (SFD submodule + SITL), 9–10 (transport + bridge), 11 (MAVLink heartbeat parsing — we know what we're connected to). Next: Tres.js drone, real WebSerial, Playwright, HTTPS+PWA. |
