...
- `src/protocol/secure-command.ts` — `SECURE_COMMAND` client: sequencing, reply matching; no session key or signing for the identity ops (decision 35). Unit-testable against fixtures; **not** SITL-testable — the handler only exists in signed builds — so bench is the integration test.
