...
- **The wizard library is the product.** Particularly the on-drone (Lua) wizards, which are what enablement protects. Operators meet it as one catalogue, **Recipes** (decision 43); "wizard" stays the internal primitive.
...
| 37 | Owner key custody | **Generated offline, backed up offline, imported into the browser as a non-extractable WebCrypto key** | Operator decision 2026-09-07. Measured on Chromium 148 before choosing: WebCrypto does X25519, a non-extractable private key performs the `deriveBits` agreement, the browser **refuses to export it**, and it survives in IndexedDB and still derives the same secret. So the tool decrypts without ever being able to read the key — which is less a loosening of decision 10 than the tool orchestrating a key it is structurally incapable of holding. Generating *in* the browser was rejected as strictly worse: nothing to back up, and clearing site data destroys every log ever written. The offline original is the backup and is the operator's to keep. WebAuthn PRF was the alternative and loses on two counts — the derived key exists transiently in page memory, and PRF support here was never verified. |
| 39 | Re-claiming a drone | **Allowed while unsealed, refused once sealed** | Operator decision 2026-09-07, revising the write-once rule F12 shipped with the same day. Write-once made a lost owner key permanent: the drone goes on writing logs nobody alive can read, with a mass erase as the only way back — a harsh penalty for losing a file, landing on the operator rather than an attacker. Sealing is where the trade flips, because a sealed drone is deployed and a re-claim there is somebody with link access re-pointing its logs at themselves. **Sealed means the silicon says so**, read from the option bytes: `BRD_OPTIONS` bit 10 is the raise-only *request*, so clearing it leaves the protection in place, and a rule resting on that parameter could be undone by a plain parameter write. Consequence, accepted: an unsealed drone can be re-claimed by anyone with link access, which widens the claiming window in the open questions below rather than closing it. |
...
| 40 | Crypto library in the tool | **`@noble/ciphers` + `@noble/hashes`, for reading `.sfx` only** | Operator decision 2026-09-07. Measured first: Chromium has X25519 in WebCrypto but **no ChaCha20 and no BLAKE2b** — only SHA-256/512 digests — and the `.sfx` envelope needs both. The alternative was moving the firmware to AES-GCM + HKDF-SHA256, rejected because an STM32H743 has no AES hardware, AES-GCM in software is materially slower than ChaCha20 on an M7, it sits on the logging IO path, and it would invalidate firmware already bench-verified. **This does not weaken decision 37**: the owner private key stays inside WebCrypto as a non-extractable key, and only the agreed shared secrets — ordinary bytes by then — reach the library. Audited, zero-dependency, pure ESM, ~10 KB gzipped for what we use. Supersedes SECURITY.md's blanket "don't import a competing crypto library", which was written when the tool performed no cryptography at all. |
| 41 | Log transfer | **MAVLink FTP with burst read, not `LOG_REQUEST_LIST` / `LOG_REQUEST_DATA`** | Revised 2026-09-07 from the Phase 4 sketch. FTP gives real filenames and sizes rather than log numbers, the client is already here and hardware-proven, and it is the same path the encrypted-log work already uses. Plain `ReadFile` costs a round trip per 239 bytes — **28.7 KB/s measured on the bench board**, which is thirty minutes for a 50 MB flight log — so `BurstReadFile` was added: the drone streams up to 2000 packets per request and **420.6 KB/s** on the same board, two minutes for that log. The trade, accepted: FTP needs a filesystem, so a board logging to internal dataflash is not covered. `LOG_REQUEST_DATA` remains the fallback if such a board ever matters. |
| 42 | Remote key exchange — how far a grant reaches | **A grant SFD signs works on a sealed drone**, with three conditions | Operator decision 2026-09-07. It is the only option that recovers a lost owner key on a deployed drone; the alternative answer is a mass erase, which punishes a customer for losing a file. The objection — that SFD can then re-point ownership silently — is weaker than it first looks: **the change announces itself**, because the owner's key stops opening new recordings at the very next download. That is a loud attack, not a stealthy one, and an SFD willing to do it would ship a firmware build instead, since it signs those too. Conditions, all three found while deciding: (1) the grant carries a **monotonic counter** and the drone stores the last applied, refusing anything not strictly newer — without it an old grant replays and restores a key the operator rotated away from; (2) the tool **shows the key fingerprint a grant would install and requires the operator to confirm it**, because the real attack is social — a customer relaying a file that points at somebody else's key; (3) the tool **says plainly when the drone's owner is not the loaded key**, making the self-announcing property explicit rather than leaving it as a decryption failure to interpret. |
...
| 31 | Bringup surface | **Ribbon** (tabs + live per-area config + inline child wizards) is the bringup wizard's DesktopView; nav "Bringup" lands on it directly. Field-install is a **property of a wizard**, not a page: a card badge, a filter on the one catalogue, and an inline **"On the radio"** toggle in the per-wizard chrome. All of them — badge, filter, toggle, header count — read the shared `useFieldToolsStore`. | Operator's mental model: *"what do I do next"* -> Bringup (the ordered chain), *"what else can I do"* -> Recipes (the catalogue). One question per surface. Settings vs procedures distinction (UX.md): simple reversible settings (ESC protocol, RPM telemetry) are inline quick controls on the ribbon's panel, not wizard steps; the order/direction check stays a wizard. **Revised 2026-09-08 (decision 43)** — this row previously described a separate `/field` page and a separate `/wizard` library; both are gone. See [docs/UX.md](docs/UX.md) Field tools / Settings-vs-procedures sections and [docs/WIZARDS.md](docs/WIZARDS.md) "Field-capable wizards". |
| 43 | Catalogue surfaces | **One catalogue. Bringup ships as free recipes inside it; the guided sequence survives as a meta-wizard.** | Operator decision 2026-09-08, prompted by watching a competing configurator bloat. The tool had grown **three catalogue surfaces for one object** — the wizard library (`/wizard`), Recipes (`/recipes`) and Field tools (`/field`) — each rendering the same manifest with different chrome and different gating. Nobody decided that; it accreted a slice at a time, which is exactly how bloat arrives. They collapse into one catalogue at `/recipes`, and bringup's steps appear in it as **free** recipes: bringup is the commoditised half of what this tool does, so charging for it buys nothing, and giving it away puts a drone on our rails — where the sealed applets and the tuning live. What does **not** collapse is the sequence. Bringup is ordered and has a done-state: motor-check before frame-select is meaningless because the frame builds the mixer, and *"this drone has never had its motors checked"* is a safety fact in a way that *"this drone hasn't had the cinewhoop recipe applied"* is not. Dissolving it into an order-free grid would bring the complexity straight back as explanatory UI telling operators what order to click in. So bringup stays a meta-wizard with its own ribbon, listed in the catalogue as the guided path, with its children individually runnable for the operator who already knows what they want. Vocabulary: **wizard** stays the internal primitive (the WIZARDS.md contract); **Recipes** is the single operator-facing name for the catalogue. |
| 44 | Guarding against complexity bloat | **A control must be justified by a recipe that could not do it. If a recipe can determine the value, there is no control.** | Operator decision 2026-09-08, alongside 43. Bloat is never decided — it is the absence of a decision. Every new capability earns a tab and no capability ever loses one, so the only thing that works is a gate on *adding*. This turns "configure, don't ask" from a bias into a test something has to pass. Three corollaries. (1) The surface most at risk is `SettingsView`, because "operator feature toggles" is a shape where a new toggle always fits — so it is narrowly defined as *a persistent property of this airframe*, and anything that is really a step in a workflow has to become a recipe instead. (2) The health metric is not how few controls we have, it is **how often an operator has to open the param browser**. High means the recipe library has a hole, and the fix is a recipe, not a control — "they can use expert mode" is never a reason to skip building one. (3) The rule constrains *asking*, not *showing*: firmware version, frame, what's calibrated, what's on the radio, the log list are read-only and are not bloat. **Generous with showing, stingy with asking.** |
| 45 | Flat before 3D | **A flat SVG schematic is the default drawing; 3D only where the answer depends on depth.** | Operator direction 2026-09-09 ("the motor styling should be simpler"), after looking at ArduConfigurator's motor map. Motor test was a three.js scene, and the cost was not the framework - it was everything the framework made necessary: a vendored quad-X mesh, a **second** procedural hub-and-arms model for every frame that mesh could not honestly represent, a spinning prop, and position labels placed in HTML by projecting world coordinates through a `PerspectiveCamera` whose fov and position had to be kept in sync with the scene **by hand, in a comment**. The flat replacement is unambiguous from any angle - which is what an operator actually needs while looking down at a drone on a bench - frame-agnostic by construction rather than by a second code path, and it draws its labels rather than projecting them. It also deleted a 750 KB vendored asset. **Simpler was also more accurate**, which is the part worth remembering: the 3D scene had to fake the frames it did not have a mesh for, and the schematic cannot. The honest case for 3D is a pose the operator has to copy with the airframe in their hands - which turned out to be a bigger case than it first looked: the Connect screen's drone now **mirrors the connected drone's attitude** (2026-09-09), so tipping the airframe checks the board's mounting and the IMU in one second with nothing asked of the operator. Sensor calibration is the other. Everything else is flat. Flat idiom and the reasoning: [docs/UX.md](docs/UX.md) "Flat before 3D". |
...
- ✅ **One catalogue** (decision 43, 2026-09-08). The wizard library, Recipes and Field tools collapse into a single catalogue at `/recipes`; bringup's steps are listed there as free recipes; the guided ordered walk stays the `bringup` meta-wizard at `/wizard/bringup`; "runs from the radio" becomes a badge + filter rather than a page.
...
- ✅ Pull .bin via **MAVLink FTP burst read** (decision 41, not the `LOG_REQUEST_*` route originally sketched): list `/APM/LOGS`, download with progress, 420 KB/s on the bench.
- ✅ **Unscramble a secured drone's recordings** — the same page opens a `.sfx` whether it came off the drone just now or off the card last week, and hands back an ordinary flight log. This half arrived early, with Phase 8.
- Save to disk via File System Access API (PWA constraint) — today it is an ordinary browser download.
...
- Enable and recovery wizards, both mountable in the bringup ribbon. **Next up:** the enable wizard's identity half is unblocked (T1, T2, T6 in); it shows the lock as pending until F6.
...
- **Should `GET_PUBLIC_KEYS` be unsigned too? (Phase 7, firmware).** It returns public data, and it is the only way the tool could confirm trust-chain link 4 — "this bootloader trusts exactly one key, and it is SFD's" — before enabling. Today it needs a signature the tool cannot produce, so the tool takes link 4 on faith from the DFU artefact it just flashed. Cheap to change (one more exemption in `signature_required()`); decide before the enable wizard's UX is designed.
- ~~**RDP lock/unlock: one `BRD_OPTIONS` bit or two?**~~ **Resolved by neither, at F6 (2026-09-04): one bit, raise-only.** Setting it asks for protection at the next boot; clearing it does nothing, because the firmware never lowers protection. That removes the hazard the two-bit design existed to dodge without needing a second bit. Left here struck through rather than deleted, since it was open for a fortnight and a reader should see how it closed.
...
- **Claiming an unsealed drone (Phase 8, firmware).** Physical presence is the
  ...
  one that is already owned, not merely claiming a fresh one. The mitigation is
  procedural and unchanged: identity, ownership and seal in one sitting, on a
  bench, and a drone must not leave it enabled but unowned. Closing it properly
  needs factory provisioning or the remote key exchange below. **Still open.**
- **Backing up the owner key is now the operator's problem (Phase 8).**
  Decision 37 puts the only recoverable copy outside the tool by design. Losing
