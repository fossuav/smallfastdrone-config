...
- **Secure your drone** (wizard library) — the SFD enable ceremony: the drone makes its own identity from its hardware random number generator, keeps the secret half for good, and you leave with the file that proves which drone it is. It branches on what the drone can actually do — ordinary ArduPilot is sent to the firmware page, a part-way-upgraded drone can update its startup software from right there. A padlock in the header and on the Connect card shows when you're talking to a secured drone.
- **Remove your drone's security** (wizard library) — the way back out, which matters as much as the way in: save the drone's settings, wipe it completely, install fresh firmware over USB, put the settings back. It states the cost first (the drone's identity is destroyed and a new one would be a different identity) and won't start until you've chosen the firmware to install afterwards, because a wiped drone with nothing to install is a drone that won't boot.
