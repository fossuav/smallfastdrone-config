...
bun run bench:bootloader         # update the board's bootloader from the running firmware's ROMFS
bun run bench:enable             # drive the SFD enable ceremony against the board
bun run bench:calibration <phase># prove the restore-path calibration force-save (see the file header)
...
- **Secure your drone** (in Recipes) — the SFD enable ceremony: the drone makes its own identity from its hardware random number generator, keeps the secret half for good, and you leave with the file that proves which drone it is. It branches on what the drone can actually do — ordinary ArduPilot is sent to the firmware page, a part-way-upgraded drone can update its startup software from right there. A padlock in the header and on the Connect card shows when you're talking to a secured drone.
- **Remove your drone's security** (in Recipes) — the way back out, which matters as much as the way in: save the drone's settings, wipe it completely, install fresh firmware over USB, put the settings back. It states the cost first (the drone's identity is destroyed and a new one would be a different identity) and won't start until you've chosen the firmware to install afterwards, because a wiped drone with nothing to install is a drone that won't boot.
- **Recipes** (`/recipes`) — the one catalogue: everything the tool can do to a drone, in one place. A banner for the guided bringup with its live step count, then a card grid grouped by what a recipe is *for* — getting flying (bringup's own steps, free, each saying which step of the guided walk it is), tuning, checks, securing — and paid entries greyed under one Pro section. Its **On the radio** filter is where you install applets onto the drone to run from your transmitter, including an encrypted one SmallFastDrone made for that airframe. The header's radio icon counts what's installed and links straight into it. Seed tuning recipes (cinewhoop, throw mode, first-flight failsafes) are still to come.
